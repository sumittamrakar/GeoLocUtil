        ...
        public async Task<ResultWrapper> Run(string[] args)
        {
            ...
            foreach (var location in locations)
            {
                ...
                if (result != null)
                {
                    ...
                    Console.WriteLine($"Location: {result.Name}, Country: {result.Country}, Latitude: {result.Lat}, Longitude: {result.Lon}");
                }
                else
                {
                    // Add an error message for the invalid location
                    var errorMessage = $"No data found for location: {location.Value}";
                    resultWrapper.Errors.Add(errorMessage);
                    Console.WriteLine(errorMessage);
                }
            }
